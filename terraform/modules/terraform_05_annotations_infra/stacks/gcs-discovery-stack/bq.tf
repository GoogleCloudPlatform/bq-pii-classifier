resource "google_bigquery_table" "view_gcs_run_summary_counts_gcs" {
  ...
  view {
    ...
    query = templatefile("../../modules/terraform_05_annotations_infra/stacks/gcs-discovery-stack/views/v_run_summary_counts_gcs.tpl",
      {
        project = var.publishing_project
...
resource "google_bigquery_table" "logging_view_label_history_gcs" {
  ...
  view {
    use_legacy_sql = false
    query = templatefile("../../modules/terraform_05_annotations_infra/stacks/gcs-discovery-stack/views/v_log_label_history_gcs.tpl",
      {
        project = var.publishing_project
        dataset = var.dlp_dataset_name
...
resource "google_bigquery_table" "dispatcher_runs_gcs_table" {
  ...
  schema = file("../../modules/terraform_05_annotations_infra/stacks/gcs-discovery-stack/schema/dispatcher_runs_gcs.json")
