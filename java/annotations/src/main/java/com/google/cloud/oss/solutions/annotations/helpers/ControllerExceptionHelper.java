...
/** Helper class to handle exceptions, check if they are retryable and return a response entity. */
public class ControllerExceptionHelper {
  ...
  public static ThrowableInfo isRetryableException(Throwable throwable) {
    ...
    if (BaseServiceException.class.isAssignableFrom(exceptionClass)) {
      ...
      return new ThrowableInfo(
          ...
          "Check: isAssignableFrom BaseServiceException");
    }
    ...
    if (ApiException.class.isAssignableFrom(exceptionClass)) {
      ...
      return new ThrowableInfo(
          throwable, apiEx.isRetryable(), "Check: isAssignableFrom BaseServiceException");
    }
  ...
  // Checks if the given throwable or recursively any of its causes are Retryable
  ...
  public static ResponseEntity handleException(
      Exception ex, LoggingHelper logger, String trackingId) {
    ...
    if (exInfo.isRetryable()) {
      ...
      if (BigQueryException.class.isAssignableFrom(exceptionClass)) {
        ...
        String msg =
            String.format(
                "BigQuery Exception: msg: %s - reason: %s - code: %s - isRetryable: %s - full"
                    + " exception: %s",
                bigQueryException.getMessage(),
                bigQueryException.getReason() != null
